...
export default function OtpStep({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await verifyOtp({ phone: session.phoneNumber, otp });
  ...
  return (
    <div className="step-panel" id="otp-step">
      ...
      <p className="step-description">
        We sent a 6-digit code to <strong>{maskedPhone}</strong>. Check your
        messages and enter it below.
      </p>

      <div
        className={`otp-expiry ${seconds <= 30 ? "expiring" : ""}`}
        id="otp-expiry"
      >
        {seconds > 0 ? (
          <>
